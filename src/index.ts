 ...
 * @returns
 */
function generateCacheGroups(subPackages: SubPackageInfo[], priority: number, enableCSS: boolean) {
  return subPackages.reduce<{ [key: string]: any }>((prev, subPackage) => {
     ...
     * @returns
     */
    function isUnderSubpackage(reasons: Set<Module>) {
    ...
    prev[subPackage.name] = {
      ...
      test(module: Module & { reasons?: Array<{ module: Module }> }) {
        ...
        if (
          // @ts-ignore
          module.miniType != null ||
          module.context?.includes('@tarojs') ||
          (!enableCSS && module.type !== 'javascript/auto')
        ) {
          return false
        }
        ...
        if (module.reasons) {
          ...
          if (reasons.size > 0 && isUnderSubpackage(reasons)) {
            cache.set(id, true)
            return true
          }
        }
...
export default function TaroPluginSubpackageOptimize(
  ctx: IPluginContext,
  options: { enableInDev?: boolean; priority?: number; enableCSS?: boolean } = {}
) {
  // 默认优先级高于 vendor 和 common
  options = { enableInDev: false, priority: 20, enableCSS: true, ...options }
  ...
  ctx.modifyWebpackChain(({ chain }) => {
    ...
    const cacheGroups = generateCacheGroups(subPackages, options.priority!, options.enableCSS!)
